...
function TimeSlots({
  ...
  const [hour12Format, setHour12Format] = useState(JSON.parse(localStorage.getItem('hour12Format')) || false);

  let remainingMonths = selectedYear === defaultYear ? months.slice(defaultMonth) : months;
  ...
  async function getCalendarDays() {
    ...
    if (res.ok) {
      const filteredDays = data.days.filter((day) => Object.keys(visitingHours).includes(day.day));
      setCalendarDays({
        ...data,
  ...
  useEffect(() => {
    async function getTimeSlots() {
      const res = await fetch(`${API_URL}/api/appointments/timeslots?doctor=${doctor_id}`, {
        method: 'GET',
        headers: {
          'content-type': 'application/json',
        },
  ...
  useEffect(() => {
    const getDailySlots = () => {
      const slots = timeSlots ? Object.entries(timeSlots).find(([day, hours]) => day === selectedDate.day) : null;
      // console.log({ slots });

      if (slots) {
        let dailySlots = slots ? Object.values(slots[1]) : [];

        const confirmedAppointments = bookedAppointments.filter((appointment) => appointment.confirmed);
        ...
        const selectedDateString = `${selectedDate.year}-${String(selectedDate.index + 1).padStart(2, '0')}-${String(
          selectedDate.date
        ).padStart(2, '0')}`;
        // console.log({ selectedDateString });

        const appointmentsForSelectedDate = confirmedAppointments.filter((appointment) => {
        ...
        appointmentsForSelectedDate.forEach((appointment) => {
          dailySlots = dailySlots.filter((slot) => slot !== appointment.time_slot);
        });
          ...
          <section className='calendar-days'>
            ...
            <div className='days-grid'>
              {calendarDays?.days?.map((item, index) => (
                <div
                  className={`calendar-items item ${selectedDate.date === item.date ? 'selected' : ''}`}
                  key={index}
                  onClick={() => handleDateClick(item.day, item.date, calendarDays.month, calendarDays.index)}>
            ...
            <section className='time-slots'>
              ...
              <div className='time-grid'>
                {dailySlots?.map((item, index) => (
                  <div
                    className={`time-slot-items item ${selectedItem === item ? 'selected' : ''}`}
                    key={index}
